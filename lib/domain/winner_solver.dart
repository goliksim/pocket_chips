...
part 'models/cards/card_combination.dart';

class WinnerSolver {
  static (int, List<Combination?>) determineWinner(
    List<List<Card?>> playerCards,
    List<Card?> table,
  ) {
    if (table.contains(null)) {
      return (0, List.filled(playerCards.length, null));
    }
  ...
  static int _findIndexOfMax(List<Combination> list) {
    if (list.isEmpty) {
